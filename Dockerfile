# Stirling-PDF-SSO — Full version with the SSO/Enterprise unlock baked in.
#
# This is the upstream Stirling-PDF "embedded" (full) build with one extra step:
# after the source is copied, the SSO overlay (sso/) is applied — new files via
# rsync and modifications via patches — before the Gradle build. The result is a
# proprietary-flavour image that runs as the ENTERPRISE tier with no paid key.
#
# Build:
#   docker build -t stirling-pdf-sso .
# Run (see sso/deploy/ for a full compose stack):
#   docker run -p 8080:8080 stirling-pdf-sso
...
RUN apt-get update \
    && apt-get install -y --no-install-recommends curl ca-certificates patch rsync \
...
# --- SSO overlay -------------------------------------------------------------
# Apply the SSO/Enterprise unlock on top of the pristine upstream tree:
#   1. apply-overlay.sh  — copy NEW files (rsync --ignore-existing)
#   2. apply-patches.sh  — patch EXISTING upstream files (unified diffs)
# Both fail the build loudly if a collision or a stale patch is detected.
RUN bash sso/script/apply-overlay.sh
RUN bash sso/script/apply-patches.sh
# The overlay sources are not part of the Gradle build; drop them from the stage.
RUN rm -rf sso

# Proprietary flavour compiles the Enterprise/SSO module; the runtime then
# auto-activates the "security" Spring profile because SecurityConfiguration is
# present on the classpath (see SPDFApplication#getActiveProfile).
...
# Metadata labels
LABEL org.opencontainers.image.title="Stirling-PDF-SSO" \
      org.opencontainers.image.description="Stirling-PDF (full) with SSO/Enterprise features unlocked — no license key required" \
      org.opencontainers.image.source="https://github.com/Deenyoro/Stirling-PDF-SSO" \
      org.opencontainers.image.licenses="MIT" \
      org.opencontainers.image.version="${VERSION_TAG}" \
      org.opencontainers.image.keywords="PDF, SSO, SAML2, OAuth2, OIDC, Enterprise, Spring Boot, React"
